...
| `hash_message_full_trace_linked_satisfies_short_and_long_seed` | Full span: 1- and 2-row seed + MGF1 trace row (step E+++) |
...
| `fold_verify_core_hash_message_seed_chain_bound_smoke` | Bound seed chain + full trace-linked core (`mlen=15`) |
| `fold_verify_core_hash_message_trace_span_plain_steps` | Plain fold over full span with trace-linked core |
| `fold_verify_core_hash_message_full_trace_linked_smoke` | `hash_message_full_span_plain` + trace-linked core (`mlen=4` → 2 steps) |
